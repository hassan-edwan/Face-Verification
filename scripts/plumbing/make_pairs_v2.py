...
Pair sampling logic is otherwise identical to v1.

Output: configs/pairs_v2.csv
"""
...
# ── CONFIG ───────────────────────────────────────────────────────────────────
LFW_PATH    = "data/lfw"
OUTPUT_PATH = "configs/pairs_v2.csv"
SEED        = 42
PAIRS_EACH  = 500
# ─────────────────────────────────────────────────────────────────────────────
...
def sample_same_pairs(id_map: dict, n: int, rng: random.Random) -> list:
    """
    Samples n same-identity pairs. Identity is chosen uniformly first,
    then two distinct image indices are sampled from that identity.
    This prevents prolific identities from dominating positive pairs.
    """
...
def sample_diff_pairs(id_map: dict, n: int, rng: random.Random) -> list:
    ...
    uniformly first, then one image index is sampled from each.
    This prevents prolific identities from dominating negative pairs.
    """
