...
RUN_ID      = "run_005_center_crop_preprocessing"
CROP_MARGIN = 0.15    # trim this fraction from each edge before resizing
TARGET_SIZE = (160, 160)
# ─────────────────────────────────────────────────────────────────────────────
...
def load_and_preprocess(path: str) -> np.ndarray:
    """
    Loads an image and applies center-crop preprocessing.
    Removes CROP_MARGIN from each edge, then resizes to TARGET_SIZE.
    """
    img = cv2.imread(path)
    if img is None:
...
# ── EMBEDDINGS & SCORES ──────────────────────────────────────────────────────
print(f"Preprocessing images with center-crop (margin={CROP_MARGIN})...")
embedder = FaceNet()
...
json_path = os.path.join(OUT_DIR, "run_005.json")
